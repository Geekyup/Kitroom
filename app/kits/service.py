...
from app.kits.exceptions import ArchiveTooLarge, KitNotReady, NotKitOwner, UploadNotFound
from app.kits.models import DrumKit, DrumKitNode, KitStatus
from app.kits.repository import KitRepository, NodeRepository
from app.kits.schemas import KitCatalogItemOut, KitOut, KitUploadInitOut
from app.kits.schemas import NodeOut
...
class KitService:
    ...
    async def confirm_kit_upload(self, kit_id: int, requester_id: int) -> DrumKit:
        ...
        if size_bytes > settings.MAX_ZIP_SIZE_MB * 1024 * 1024:
            ...
            await self.kit_repo.update_status(
                kit.id, KitStatus.FAILED, error_message="Файл превышает максимальный размер"
            )
            raise ArchiveTooLarge()
    ...
    async def list_catalog(self, limit: int = 50, offset: int = 0) -> list[KitCatalogItemOut]:
        ...
        return [await self._to_catalog_item(kit) for kit in kits]
    ...
    async def _to_catalog_item(self, kit: DrumKit) -> KitCatalogItemOut:
        return KitCatalogItemOut(
            id=kit.id,
            title=kit.title,
            slug=kit.slug,
